...
function initdata() {
	...
	for (var i=0; i < str.length; i++) {
		...
		if (str[i] != "") {
			...
			var name=addata[2];
			if(i < 5){
				huileftParas[i].url = ahref;
				huileft[i].getElementsByTagName("img")[0].src = pic;
				//huileft[i].getElementsByTagName("span")[0].innerHTML = name;
			}else{
				huirightParas[i - 5].url = ahref;
				huiright[i - 5].getElementsByTagName("img")[0].src = pic;
				huiright[i - 5].getElementsByTagName("span")[0].innerHTML = name;	
			}
		}
