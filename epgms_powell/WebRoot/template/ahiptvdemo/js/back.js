...
function getbackurl(){
	...
	for(var i=0;i<arrCookie.length;i++){
		...
		if(cookieValue.indexOf("liuyang") >= 0){
			...
			if(strurls[1] != ""){
				...
				//重新设置返回地址的值
				if(index == -1){
					document.cookie = "liuyang="+"; path=/";
				}else{
					document.cookie = "liuyang=" + strurls[1].substring(0, index)+"; path=/";
				}
				break;
...
//	alert("pageurl--"+pageurl);
	var strCookie = document.cookie;
	var index = strCookie.indexOf("liuyang");
...
//		alert("setpage if index="+index);
		...
		for(var i=0;i<arrCookie.length;i++){
			...
			var strurls = cookieValue.split("=");
			var temp = strurls[0];
			if(temp.indexOf('liuyang') >= 0){//判断是否是pageurl
			  var backurls = strurls[1];
			  backurls = backurls + "|" + pageurl;//追加返回地址值
			  document.cookie = "liuyang=" + backurls+"; path=/";
			  break;
			}
		}
	}else{
//		alert("setpage else index="+index);
		document.cookie = "liuyang=" + pageurl+"; path=/";
	}
}